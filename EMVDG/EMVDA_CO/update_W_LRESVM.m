function [W,G,obj] = update_W_LRESVM(Z, G, pos_ftr, neg_ftr, L, M, param)
...
p0_all = 2*param.lambda1*diag(train_label)*train_ftr*L; 
...
%     obj = sum(loss_weight*max((1-diag(train_label)*train_ftr*W).*loss_mask,0)) + 1/2*trace(W'*W) + param.lambda2*norm(G-G*Z,'fro')^2 + param.lambda1*norm(G-W,'fro')^2 + param.theta*trace(W'*M*W);
