...
{
	mexPrintf(
	"Usage: model = svmtrain(p, training_label_vector, training_instance_matrix, 'libsvm_options');\n"
	"libsvm_options:\n"
...
//read linear term  after read problem
int read_problem_linear_term(const mxArray *p_vec)
{
   int i;
   double *p = NULL; 
   int rows, cols;
   ...
   if (prob.l != rows || cols != 1)
   { 
      mexPrintf("Error: wrong format for linear term p");
      return 1;
   }

   prob.p = Malloc(double,prob.l);
		...
		int nrhs, const mxArray *prhs[] )
{
	...
	if(nrhs > 2 && nrhs < 5)
	{
		...
		if(!mxIsDouble(prhs[0]) || !mxIsDouble(prhs[1]) || !mxIsDouble(prhs[2])) {
			mexPrintf("Error: linear term p, label vector and instance matrix must be double\n");
			fake_answer(plhs);
			return;
		}
        ...
        {
            mexPrintf("Error: s=%d is not supported, only C_SVC(0), ONE_CLASS(2) are supported currently\n", param.svm_type);
            svm_destroy_param(&param);
            fake_answer(plhs);
            return;
        }
		...
		{
            mexPrintf("Error: only dense format instance is supported currently\n");
            svm_destroy_param(&param);
            fake_answer(plhs);
            return;
			...
			if(param.kernel_type == PRECOMPUTED)
			{
				...
				err = read_problem_dense(prhs[1], lhs[2]);
			...
			err = read_problem_dense(prhs[1], prhs[2]);
        err = read_problem_linear_term(prhs[0]);
