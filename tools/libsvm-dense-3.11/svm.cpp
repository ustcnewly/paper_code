	...
	double *alpha, Solver::SolutionInfo* si, double Cp, double Cn)
{
	...
	Solver s;
	s.Solve(l, SVC_Q(*prob,*param,y), prob->p, y,
	...
	double *alpha, Solver::SolutionInfo* si)
{
	...
	Solver s;
	s.Solve(l, ONE_CLASS_Q(*prob,*param), prob->p, ones,
		alpha, 1.0, 1.0, param->eps, si, param->shrinking);
	...
	else
	{
		...
		for(i=0;i<nr_class;i++)
			for(int j=i+1;j<nr_class;j++)
			{
				...
				sub_prob.p = Malloc(double,sub_prob.l);
				int k;
				for(k=0;k<ci;k++)
				{
					sub_prob.x[k] = x[si+k];
					sub_prob.p[k] = pp[si+k];
					sub_prob.y[k] = +1;
				}
				for(k=0;k<cj;k++)
				{
					sub_prob.x[ci+k] = x[sj+k];
					sub_prob.p[ci+k] = pp[sj+k];
					sub_prob.y[ci+k] = -1;
				}
