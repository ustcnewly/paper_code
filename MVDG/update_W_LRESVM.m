...
w_part2 = 1/(1+2*param.lambda12)*train_ftr'*diag(train_label);
Q_all = 1/(1+2*param.lambda12)*(train_ftr*train_ftr').*(train_label*train_label');
p0_all = 2*param.lambda12/(1+2*param.lambda12)*diag(train_label)*train_ftr; 
...
for iter = 1:LRESVM_MAX_ITER
    ...
    % Update W
    p_all = p0_all*G;
    w_part1 = 2*param.lambda12/(1+2*param.lambda12)*G;
    ...
    obj = sum(loss_weight*max((1-diag(train_label)*train_ftr*W).*loss_mask,0)) + 1/2*trace(W'*W) + param.lambda11*norm(G-G*Z,'fro')^2 + param.lambda12*norm(G-W,'fro')^2;
