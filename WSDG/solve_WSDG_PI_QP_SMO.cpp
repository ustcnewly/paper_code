...
mxArray* solve_qp_smo(const mxArray *combo_vkernel_mx, const mxArray *combo_tkernel_mx, const mxArray *group_list_mx, const mxArray *bag_list_mx, const mxArray *cate_list_mx, const mxArray *domain_list_mx,\
        ...
        const mxArray *total_bag_num_mx, const mxArray *domain_num_mx, const mxArray *gamma_mx, const mxArray *max_iter_mx, const mxArray *eps_top_mx, const mxArray *quiet_mode_mx)
{   
    int ii,iter,ismpl;
    ...
    double *group_list = mxGetPr(group_list_mx); 
    ...
    double *domain_max_val = (double*)malloc(2*total_bag_num*sizeof(double));       
    int *domain_max_idx = (int*)malloc(2*total_bag_num*sizeof(int));
    
    // prepare for choosing j
    double *domain_obj_val = (double*)malloc(2*total_bag_num*sizeof(double));
    double *domain_diff = (double*)malloc(2*total_bag_num*sizeof(double));
    int *domain_min_idx = (int*)malloc(2*total_bag_num*sizeof(int));
        
    for(iter=0; iter<SMO_MAX_ITER; iter++)
    {
        // initialize
        for(int kk=0;kk<2*total_bag_num;kk++)
        {
        ...
        for(ii=0; ii<st_num; ii++)
        {
            ...
            tmpj = ii;
            tmp_ss = return_H(combo_vkernel, combo_tkernel, bag_list, cate_list, domain_list, d_arr, nd, total_bag_num, domain_num, st_num, gamma, tmpi, tmpi);
            tmp_tt = return_H(combo_vkernel, combo_tkernel, bag_list, cate_list, domain_list, d_arr, nd, total_bag_num, domain_num, st_num, gamma, tmpj, tmpj);
            tmp_st = return_H(combo_vkernel, combo_tkernel, bag_list, cate_list, domain_list, d_arr, nd, total_bag_num, domain_num, st_num, gamma, tmpi, tmpj);
        ...
        min_domain_obj_val = min_vec(domain_obj_val, 2*total_bag_num, domain_min_idx, tmp_domain_idx);

        update_i = domain_max_idx[tmp_domain_idx];
        update_j = domain_min_idx[tmp_domain_idx];
        
        if(update_i==-1 || update_j==-1)
        {
            break;
        }
        ...
        max_domain_diff = max_vec(domain_diff, 2*total_bag_num, domain_min_idx, tmp_domain_idx);
        ...
        for(ii=0;ii<st_num;ii++)
        {
            tmpi = ii;
            tmpj = update_i;
            tmp_ss = return_H(combo_vkernel, combo_tkernel, bag_list, cate_list, domain_list, d_arr, nd, total_bag_num, domain_num, st_num, gamma, tmpi, tmpj);
            tmpj = update_j;
            tmp_tt = return_H(combo_vkernel, combo_tkernel, bag_list, cate_list, domain_list, d_arr, nd, total_bag_num, domain_num, st_num, gamma, tmpi, tmpj);
...
{   
    plhs[0] = solve_qp_smo(prhs[0], prhs[1], prhs[2], prhs[3], prhs[4], prhs[5], prhs[6], prhs[7], prhs[8], prhs[9], prhs[10], prhs[11], prhs[12], prhs[13], prhs[14], prhs[15], prhs[16]);
}	
