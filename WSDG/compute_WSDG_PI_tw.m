function w_arr = compute_WSDG_PI_tw(combo_h_ftr, bag_list, cate_list, domain_list, alpha, theta, param)
...
for iter = 1:param.h_iter
    ...
    for ci = 1:param.cate_num
        for dmi = 1:param.domain_num
            ...
            w_seg = alpha_theta(sub_idx)'*combo_ftr((bag_idx-1)*param.domain_num+dmi,:);
            % bottom part
            sub_cate_idx = find(cate_list==ci);
            sub_domain_idx = find(domain_list==dmi);
            sub_idx = intersect(sub_cate_idx, sub_domain_idx);
            bag_idx = bag_list(sub_idx);
            w_seg = w_seg + alpha_theta(sub_idx)'*combo_ftr(param.domain_num*param.total_bag_num+bag_idx,:);           
