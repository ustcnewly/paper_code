function [model, obj, history] = DAMMC_cutting_plane_fast2_sm(data, opt)
%
% Inputs
% data.
        ...
        if l > 0
            ...
            for s = 1 : S
                ...
                if 1
                    ...
                    y_set_tmp(idx_t(sidx(1:np)),s) = 1;
                    y_set_tmp(idx_t(sidx(np+1:end)), s) = -1;
                else
                    tt = sign(dv);
                    if all(tt==-1)
                        [~,m_idx] = max(dv);
                        tt(m_idx) = 1;
                        y_set_tmp(idx_t,s) = tt;
                    end
                end
                y_set_tmp(idx_l,s)=y(idx_l);
            end
            yy = y_set_tmp;
            yy = unique(yy', 'rows')';
        else
