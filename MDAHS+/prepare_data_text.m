function data_tmp = prepare_data_text(data, options)
...
Y = cell(S,1);
X = cell(S,3); % three types of features: 2D(SIFT), 3D(HOG, HOF, MBH), text

offset_ = 0;
for s = 1 : 2
    Y{s,1} =  data.domains{s}.labels;
    X{s,1} =  data.domains{s}.feat_2D.decaf;
    X{s,3} =  data.domains{s}.feat_text;
...
K = cell(S,S,3);% three types of features;
