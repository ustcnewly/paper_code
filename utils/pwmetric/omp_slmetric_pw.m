function M = omp_omp_slmetric_pw(X1, X2, mtype, varargin)
tic;
%omp_slmetric_PW Compute the metric between column vectors pairwisely
%
% [ Syntax ]
%   - M = omp_slmetric_pw(X1, X2, mtype);
%   - M = omp_slmetric_pw(X1, X2, mtype, ...);
%
% [ Arguments ]
...
%
% [ Description ]
%    - M = omp_slmetric_pw(X1, X2, mtype) Computes the metrics between
...
%
%    - M = omp_slmetric_pw(X1, X2, mtype, ...) Some metric types requires
...
%         M = omp_slmetric_pw(X1, X2, 'eucdist');
%
%         % compute the eucidean distances between the samples
%         % in X1 in a pairwise manner
%         M = omp_slmetric_pw(X1, X1, 'eucdist');
%
%         % compute the city block distances (L1)
%         M = omp_slmetric_pw(X1, X2, 'cityblk'); 
%
%         % compute the normalize correlations
%         M = omp_slmetric_pw(X1, X2, 'nrmcorr');
%
%         % compute hamming distances
%         M = omp_slmetric_pw(X1, X2, 'hamming', 0.5);
%         M2 = omp_slmetric_pw((X1 > 0.5), (X2 > 0.5), 'hamming');
%         assert(isequal(M, M2));
%     \}
%
...
%         weights = rand(10, 1);
%         M = omp_slmetric_pw(X1, X2, 'wsqdist', weights);
%
%         % compute quadratic distances (x-y)^T * Q (x-y)
%         Q = rand(10, 10);
%         M = omp_slmetric_pw(X1, X2, 'quaddiff', Q);
%
%         % compute Minkowski distance of order 3         
%         M = omp_slmetric_pw(X1, X2, 'minkowski', 3);
%     \}
%
% [ History ]
...
assert(ischar(mtype), 'sltoolbox:omp_slmetric_pw:invalidarg', ...
...
if strcmp(mtype, 'hamming') || strcmp(mtype, 'hamming_nrm')
    assert((isnumeric(X1) || islogical(X1)) && ndims(X1) == 2 && ...
        ...
        'sltoolbox:omp_slmetric_pw:invalidarg', 'X1 and X2 should be numeric or logical matrices.');        
else
    assert(isnumeric(X1) && ndims(X1) == 2 && isnumeric(X2) && ndims(X2) == 2, ...
        'sltoolbox:omp_slmetric_pw:invalidarg', 'X1 and X2 should be numeric matrices.');
end

assert(isa(X2, class(X1)), ...
    'sltoolbox:omp_slmetric_pw:invalidarg', 'X1 and X2 should be of the same class.');
...
switch mtype        
    ...
    case 'cityblk'
        checkdim(X1, X2);  
        M = omp_pwmetrics_cimp(X1, X2, int32(1));
                        
    case 'maxdiff'
        checkdim(X1, X2); 
        M = omp_pwmetrics_cimp(X1, X2, int32(3));
        
    case 'mindiff'
        checkdim(X1, X2);  
        M = omp_pwmetrics_cimp(X1, X2, int32(2));
        
    case 'minkowski'
        checkdim(X1, X2);
        pord = varargin{1};
        if ~isscalar(pord)
            error('sltoolbox:omp_slmetric_pw:invalidparam', ...
                'the mikowski order should be a scalar');
        end
        pord = cast(pord, class(X1));        
        M = omp_pwmetrics_cimp(X1, X2, int32(4), pord);
        ...
        if islogical(X1) && islogical(X2)
            ...
            if isempty(varargin)
                ...
                assert(isnumeric(t) && isscalar(t), ...
                    'sltoolbox:omp_slmetric_pw:invalidparam', 't should be a numeric scalar.');
            end
            H1 = X1 > t;
            H2 = X2 > t;
        end
    ...
    case 'intersect'
        checkdim(X1, X2);
        M = omp_pwmetrics_cimp(X1, X2, int32(5));
        
    case 'intersectdis'
        checkdim(X1, X2);
        M = 1 - omp_pwmetrics_cimp(X1, X2, int32(5));
        
    case 'chisq'
        checkdim(X1, X2);
        M = omp_pwmetrics_cimp(X1, X2, int32(6));
        
    case 'kldiv'
        checkdim(X1, X2);
        M = omp_pwmetrics_cimp(X1, X2, int32(7));
        
    case 'jeffrey'
        checkdim(X1, X2);
        M = omp_pwmetrics_cimp(X1, X2, int32(8));
        
    otherwise
        error('sltoolbox:omp_slmetric_pw:unknowntype', 'Unknown metric type %s', mtype);
...
d = size(X1, 1);
if d ~= size(X2, 1)
    error('sltoolbox:omp_slmetric_pw:sizmismatch', ...
